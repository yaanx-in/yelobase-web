  ...
  return (
    <div className={cn("flex flex-wrap gap-2", className)}>
      {tags.map((t) => (
        <span
          key={t}
          className="rounded-[6px] border border-[var(--color-border-subtle)] bg-[var(--color-background)] px-2 py-1 text-xs font-medium text-[var(--color-text-secondary)]"
        >
          {t}
        </span>
      ))}
    ...
    <div className={cn("flex items-center gap-2.5", className)}>
      {author.imageUrl ? (
        <Image
          ...
          width={32}
          height={32}
          className="size-9 shrink-0 rounded-[12px] border-2 border-[var(--color-text-primary)] object-cover"
        />
      ) : (
        <span
          aria-hidden
          className={`inline-flex size-9 shrink-0 items-center justify-center rounded-[12px] border-2 border-[var(--color-text-primary)] text-[11px] font-bold text-white ${coverClass(post.accent)}`}
        >
